...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', default='./config/config_BoT_AICUP.yaml', help='Config Path')
    ...
    parser.add_argument('--model_arch', default='MBR_4B', help='Model Architecture')
    ...
    parser.add_argument('--path_weights', default=None, help='Use of mixed precision') 
    ...
    print("Total number of query samples:", len(data_q.dataset))
    # Create Model

    path_weights = args.path_weights
    model = get_model(data, device)  # new model with class_num = 3441

    # Load the checkpoint state_dict
    checkpoint = torch.load(path_weights, map_location='cpu')

    # Create a new dictionary without the mismatched classifier layers.
    new_checkpoint = {}
    for k, v in checkpoint.items():
        # Skip keys that correspond to the classifier weights and biases in the finalblocks.
        if "finalblock.finalblocks" in k and ("classifier.0.weight" in k or "classifier.0.bias" in k):
            continue
        new_checkpoint[k] = v

    # Load the filtered state_dict into the model; strict=False allows missing keys.
