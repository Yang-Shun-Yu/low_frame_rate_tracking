...
def test_epoch(model, device, dataloader_q, dataloader_g, model_arch, writer, epoch, remove_junk=True, scaler=False):
    ...
    #             _, _, ffs, _ = model(image, cam_id, view_id)
    #         print("ffs:", ffs)
    #         # Check if ffs is non-empty
    #         if not ffs:
    #             print("Warning: ffs is empty for this batch.")
    #             continue
            
    #         end_vec = []
    #         for item in ffs:
    #             norm_item = F.normalize(item)
    #             if norm_item.numel() > 0:
    #                 end_vec.append(norm_item)
            
    #         if not end_vec:
    #             print("Warning: No normalized features in this batch, skipping.")
    #             continue
    ...
    # After the loop, check if qf is empty:
    # if not qf:
    #     raise ValueError("No query features were extracted. Please check your dataloader and model outputs.")
